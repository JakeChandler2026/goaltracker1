...
begin
  if not exists (select 1 from pg_type where typnamespace = 'public'::regnamespace and typname = 'app_role') then
    create type public.app_role as enum ('youth', 'youth_leader', 'bishop', 'parent', 'administrator');
  end if;
end
$$;
...
create index if not exists idx_parent_youth_links_youth_id on public.parent_youth_links(youth_id);
